...
export function DemoShowcaseSection() {
  ...
  return (
    <section id="demo" className="py-16 md:py-24 bg-[#090d16] text-white relative overflow-hidden border-t border-b border-white/[0.08]">
      <div className="mx-auto max-w-6xl px-4 sm:px-6 space-y-10">
        {/* Section Header */}
        <div className="text-center max-w-2xl mx-auto space-y-3">
          <div className="inline-flex items-center gap-1.5 rounded-full bg-indigo-500/10 border border-indigo-500/20 px-3 py-1 text-xs font-mono text-indigo-300">
            <Terminal className="h-3.5 w-3.5" />
            <span>DEVELOPER WORKSPACE CAPABILITIES</span>
          </div>
          <h2 className="text-2xl sm:text-4xl font-extrabold tracking-tight text-white">
            Designed for Engineers, Grounded in Real Source
          </h2>
        ...
        <div className="flex justify-center px-2">
          <div className="inline-flex max-w-full items-center overflow-x-auto no-scrollbar rounded-xl bg-[#0e1424] p-1 border border-white/[0.08] shadow-inner gap-1">
            <button
              onClick={() => setActiveTab('chat')}
              className={`flex items-center gap-1.5 sm:gap-2 rounded-lg px-3 sm:px-4 py-2 text-xs font-semibold whitespace-nowrap transition ${
                activeTab === 'chat'
            ...
            >
              ...
              <span><span className="hidden sm:inline">Split-View </span>Chat + Monaco</span>
            </button>
            <button
            ...
            >
              ...
              <span>Architecture<span className="hidden sm:inline"> Overview</span></span>
            </button>
            <button
            ...
            >
              ...
              <span>Bug <span className="hidden sm:inline">Investigation</span><span className="sm:hidden">Locator</span></span>
            </button>
          </div>
        </div>

        {/* Active Mockup Container */}
        <div className="rounded-2xl border border-white/[0.08] bg-[#0c101d] p-4 sm:p-6 shadow-2xl">
          {activeTab === 'chat' && (
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
              {/* Left Pane: Chat Mockup */}
              <div className="lg:col-span-5 rounded-xl border border-white/[0.08] bg-[#0e1526] p-4 space-y-4 flex flex-col justify-between">
                <div className="space-y-3">
                  ...
                  <div className="flex justify-start">
                    <div className="rounded-xl bg-[#131b30] border border-white/[0.08] p-3.5 text-xs text-slate-200 space-y-2 max-w-[95%]">
                ...
                <div className="rounded-lg border border-white/[0.06] bg-black/30 p-2 text-center text-[10px] text-slate-400 font-mono">
                  Grounded in actual repository AST line ranges
                </div>
              </div>

              {/* Right Pane: Monaco Editor Mockup */}
              <div className="lg:col-span-7 rounded-xl border border-white/[0.08] bg-[#070a12] p-4 space-y-3 font-mono text-xs">
                <div className="flex items-center justify-between border-b border-white/[0.08] pb-2">
                  ...
                  <span className="rounded bg-emerald-500/10 px-2 py-0.5 text-[10px] text-emerald-400 border border-emerald-500/20">
                    Lines 35&ndash;62 Highlighted
                  </span>
                </div>

                {/* Code Window with Highlighted Lines */}
                <div className="rounded-lg bg-black/40 p-4 space-y-1 overflow-x-auto text-[11px] leading-relaxed">
                  <div className="text-slate-600">34 |   @Override</div>
                  <div className="bg-indigo-950/80 border-l-2 border-indigo-400 px-2 py-0.5 text-indigo-200">
                ...
                <div className="flex items-center justify-between text-[11px] text-slate-400 pt-1">
                  ...
                  <span className="text-indigo-400">Read-Only Syntax Highlighting</span>
                </div>
              </div>
            </div>
          )}
          ...
          {activeTab === 'architecture' && (
            <div className="space-y-4">
              <div className="rounded-xl border border-white/[0.08] bg-[#0e1526] p-5 space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
                  <div className="rounded-xl bg-black/40 p-4 border border-white/[0.06] space-y-2">
                  ...
                  <div className="rounded-xl bg-black/40 p-4 border border-white/[0.06] space-y-2">
                    ...
                    <p className="text-slate-400 text-[11px] leading-relaxed">
                      Gemini models generate citation-enforced answers deep-linked into the split-view Monaco Editor.
                    </p>
                  </div>
                </div>
              </div>
          ...
          {activeTab === 'bug' && (
            <div className="space-y-4">
              <div className="rounded-xl border border-white/[0.08] bg-[#0e1526] p-5 space-y-4">
                ...
                <div className="rounded-xl bg-black/40 p-4 border border-white/[0.06] text-xs space-y-2">
                  <div className="font-semibold text-rose-300">Suspected Root Cause:</div>
                  <p className="text-slate-300 leading-relaxed text-[11px]">
                    <code className="text-rose-400 font-mono">NullPointerException</code> at <span className="underline cursor-pointer text-indigo-300 font-mono">AuthService.java:42</span> caused by unverified OAuth user payload when GitHub email visibility is set to private.
                  </p>
                  <div className="pt-2 font-semibold text-emerald-400 font-mono">Proposed Code Fix:</div>
                  <pre className="rounded-lg bg-slate-950 p-3 font-mono text-[11px] text-emerald-300 overflow-x-auto border border-white/[0.05]">
